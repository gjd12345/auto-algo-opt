...
- From the current end city choose a nearest unvisited city
- Repeat until all cities are visited; do not close the tour

Why selected: Same NN construction used as a Hamiltonian path (no return).

## 适用条件、假设与限制

Subproblem: Open TSP / Hamiltonian path — Path visits each city once, no return.

Official tsp_construct still closes the tour; treat this as a scoring idea only.
...
- mappable: possible
- not_the_original: Official tsp_construct still closes the tour; treat this as a scoring idea only.
