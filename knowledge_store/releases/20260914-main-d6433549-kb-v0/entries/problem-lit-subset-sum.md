# Subset sum (literature secondary)
...
Catalog search query: `subset sum heuristic`.
...
Target heuristics (search hints only): Greedy subset sum (not_mappable), Differencing subset sum (not_mappable).
