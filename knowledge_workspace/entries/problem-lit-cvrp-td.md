# Time-dependent VRP (literature secondary)

Secondary literature variant, not a new registered EoH problem_id.

## 定义或方法步骤

Travel time varies with clock.

Catalog search query: `time dependent VRP heuristic`.
...
Target heuristics (search hints only): TD NN (not_mappable), TD local search (not_mappable).
